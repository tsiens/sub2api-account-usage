...
const FLOAT_BAR_SIZE = { width: 360, height: 34 };
const DEFAULT_CONFIG = { baseUrl: '', updateInterval: 300, rotationInterval: 5, requestTimeout: 15000, allowInsecureTls: false, showFloatingBar: true };
...
class AppStore {
  constructor() {
    ...
    this.data = { config: { ...DEFAULT_CONFIG }, secrets: {} };
    this.load();
  }
...
function createFloatBar() {
  ...
  floatBar.setAlwaysOnTop(true, 'floating');
...
function positionFloatBar() {
  ...
  floatBar.setPosition(Math.round(area.x + (area.width - bounds.width) / 2), area.y + 18, false);
}

function syncFloatingBar(enabled) {
  if (enabled) {
    createFloatBar();
    positionFloatBar();
...
function restartRefreshTimer() {
  ...
  refreshTimer = setInterval(() => void refreshUsage(false), service.getConfig().updateInterval * 1000);
}
...
function registerIpc() {
  ...
  ipcMain.handle('save-config', async (_event, values) => {
    ...
    restartRefreshTimer();
    restartRotation();
    syncFloatingBar(config.showFloatingBar);
    updateTrayMenu();
    broadcastState();
    return config;
  });
  ...
  ipcMain.on('move-float', (event, delta = {}) => {
    ...
    floatBar.setPosition(Math.round(x + dx), Math.round(y + dy), false);
  });
...
async function initialize() {
  ...
  service = new UsageService({ store, logger: (message) => appendLog(message) });
  registerIpc();
  createPanel();
  createTray();
  state.authMode = await service.getAuthMode();
  syncFloatingBar(service.getConfig().showFloatingBar);
  restartRefreshTimer();
